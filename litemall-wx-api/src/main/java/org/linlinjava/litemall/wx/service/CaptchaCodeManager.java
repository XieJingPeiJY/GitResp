...
public class CaptchaCodeManager {
    ...
    public static boolean addToCache(String phoneNumber, String code) {
        ...
        if (captchaCodeCache.get(phoneNumber) != null) {
            if (captchaCodeCache.get(phoneNumber).getExpireTime().isAfter(LocalDateTime.now())) {  //系统时钟获取当前日期时间
                return false;    //false不用重新设置验证码
            } else {
                //存在但是已过期，删掉
                captchaCodeCache.remove(phoneNumber);  //过期，captchaCodeCache移除手机号
            }
        }
