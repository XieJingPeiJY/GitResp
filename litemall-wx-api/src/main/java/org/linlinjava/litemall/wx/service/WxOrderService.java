...
public class WxOrderService {
    ...
    //跳过验证，直接支付成功
    public Object prepay2(Integer userId, String body, HttpServletRequest request) {
        Integer orderId = JacksonUtil.parseInteger(body, "orderId");
        LitemallOrder order = orderService.findById(userId, orderId);
        System.out.println(
                order.getComments()+"GGG"+
                        order.getOrderStatus()+"GGG"+
                        order.getId()+"GGG"+
                        order.getActualPrice()+"GGG"
        );
        //点击去付款，直接改转态成已付款
     ...
     * @return 支付订单ID
     */
    @Transactional//表示全部方法开启事务，也可放在某个方法上面，表示这个方法开启事务，一般都放在service层里 详情百度
    public Object prepay(Integer userId, String body, HttpServletRequest request) {
        ...
        try {
            ...
            System.out.println(""+orderRequest.getOutTradeNo()+"**"+orderRequest.getOpenid()+"**"+orderRequest.getBody()+"**"+orderRequest.getTotalFee()+"**"+orderRequest.getSpbillCreateIp());

            result = wxPayService.createOrder(orderRequest);//第三方接口返回的参数,到前端发起支付请求
        } catch (Exception e) {
            e.printStackTrace();
        ...
        //修改订单转态并做判断
        if (orderService.updateWithOptimisticLocker(order) == 0) {  //orderService.updateWithOptimisticLocker(order)  修改订单状态(待付款为1，待发货为2，待收货为3，待评价为4
