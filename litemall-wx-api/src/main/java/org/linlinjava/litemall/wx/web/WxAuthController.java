...
public class WxAuthController {
    ...
    public Object login(@RequestBody String body, HttpServletRequest request) {
        ...
        if (username == null || password == null) {
            return ResponseUtil.badArgument();     //想前端发送“参数不对”信息    发送到前端的格式  obj.put("errno", errno);
        ...
        if (userList.size() > 1) {               //查出 大于两条id   sql  报 “系统内部错误”  （理论上一条id查出一条sql）//理论上只有1个用户名
            return ResponseUtil.serious();
        } else if (userList.size() == 0) {
            return ResponseUtil.fail(AUTH_INVALID_ACCOUNT, "账号不存在");
        } else {
            user = userList.get(0);      //查出正常对应的sql
        }

        BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
        if (!encoder.matches(password, user.getPassword())) {    //底层密码异或比对          // int a = 5; // 0000 0101      异或  相同为0  不同为1
                                                                            //int b = 3; // 0000 0011
                                                                           // a |= b; // 0000 0111//核对账号的密码
//                                                                            int a = 5; // 0000 0101
//                                                                            int b = 3; // 0000 0011
//                                                                            a &= b; // 0000 0001
//                                                                            int a = 5; // 0000 0101
//                                                                            int b = 3; // 0000 0011
//                                                                            a ^= b; // 0000 0110
            return ResponseUtil.fail(AUTH_INVALID_ACCOUNT, "账号密码不对");//没有经过此方法，说明验证通过
        }

        // 更新登录情况
        user.setLastLoginTime(LocalDateTime.now());//设置登录时间
        user.setLastLoginIp(IpUtil.getIpAddr(request));  //设置ip
        if (userService.updateById(user) == 0) {     //更数据  存进数据库（存登录时间和ip地址） user.setUpdateTime(LocalDateTime.now());.....成功为1，不成功为0
        ...
        userInfo.setAvatarUrl(user.getAvatar());    //获得用户数据库信息

        // token
        String token = UserTokenManager.generateToken(user.getId());//根据username  查出的id 存进token

        Map<Object, Object> result = new HashMap<Object, Object>();
        result.put("token", token);                    //发送token
        result.put("userInfo", userInfo);             //发送用户信息
    ...
    public Object loginByWeixin(@RequestBody WxLoginInfo wxLoginInfo, HttpServletRequest request) {
        String code = wxLoginInfo.getCode();//验证code
        ...
        String openId = null;
        try {
            WxMaJscode2SessionResult result = this.wxService.getUserService().getSessionInfo(code);//调用微信外部接口解析code，获得信息
            sessionKey = result.getSessionKey();//从code中获取sessionKey
            openId = result.getOpenid();//从code中获取openId
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        LitemallUser user = userService.queryByOid(openId);//从外部接口获取openid，然后查询是否有本地数据信息，没有则添加信息
        if (user == null) {
            ...
            user.setLastLoginIp(IpUtil.getIpAddr(request));//获取IP
