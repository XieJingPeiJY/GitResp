...
public class WxHomeController {
     ...
     */
    @GetMapping("/index")
    public Object index(@LoginUser Integer userId) {            //小程序首页
        //优先从缓存中读取
        ...
        Callable<List> bannerListCallable = () -> adService.queryIndex();          //  Callable线程 java 多线程返回list对象

        Callable<List> channelListCallable = () -> categoryService.queryChannel();

        Callable<List> couponListCallable;
        if(userId == null){      //没有登录状态
...
//    private static ThreadPoolExecutor executorService = new ThreadPoolExecutor(9, 9, 1000, TimeUnit.MILLISECONDS, WORK_QUEUE, HANDLER);
        ...
        try {
            List list = newGoodsListTask.get();
        ...
        try {
            entity.put("banner", bannerTask.get());        //轮播图数据   banner.length>0才会显示dom树
            entity.put("channel", channelTask.get());      //居家，餐厨多选项等模块
           // entity.put("couponList", couponListTask.get());    //优惠券模块
            entity.put("newGoodsList", newGoodsListTask.get());       //新品模块
            entity.put("hotGoodsList", hotGoodsListTask.get());      //人气推荐模块，以下都是对应的模块
...
//        }
    ...
    private FutureTask<List> getNewGoodsListTask(FutureTask<List> newGoodsListTask) {
        return newGoodsListTask;
    }

    private FutureTask<List> getTask(FutureTask<List> bannerTask) {
        return bannerTask;
    }
